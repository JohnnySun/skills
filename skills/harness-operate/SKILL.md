...
**開工第一步（認領鎖）**：先 `git pull --rebase` 拉齊 → 把選中條目改成
`status: consuming(<今日 YYYY-MM-DD>)` 單獨 commit → **立即 push**。鎖不 push
就不是鎖：多人/多機的互斥全靠這個 push。被拒先 pull --rebase：**你認領的
那行起衝突＝撞鎖，換下一條**；無衝突只是別人動了別處，重新 push 即可
（注意：inbox 這類狀態行原地改的檔**不得**標 merge=union——union 會把撞鎖
衝突靜默吃掉，鎖就退化成自覺）。此標記同時是當日消費打卡
