...
1. **鎖不 push 就不是鎖**。任何用 repo 檔案當併發鎖/打卡的機關（如 inbox 的
   consuming 標記），認領協議必須是 `pull → 認領 commit → 立即 push`；
   push 被拒先 pull --rebase：**認領行起衝突＝撞鎖換單元**，無衝突＝別人動了
   別處、重新 push。push 前的競態窗口接受（殘餘風險是偶爾重複勞動，不是事故）。
