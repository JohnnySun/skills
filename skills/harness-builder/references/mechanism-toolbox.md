...
1. **鎖不 push 就不是鎖——且鎖的形態要匹配推送模式**。
   ...
   - 嚴格 CR 型（主幹受保護不可直推）：改用**分支/ref 創建的原子性**當鎖
     ...
     打卡判定用 `git ls-remote` 查當日 claim 分支（惰性，本地查不到才走
     網路）；狀態行記帳隨工作 PR 走。**刪除時機：當日保留（兼打卡標記）、
     次日回收**——已合併的刪分支，未合併的是殭屍（刪分支＋條目回隊列）。
     若連建分支都被限制，退用 forge API 的原子物件（issue 自我指派、label）。
