...
## 消費端機械化：anacron 派工（治所有 harness 的頭號死因）

收集端天然是機械的（hook 強制寫入），消費端若靠「行有餘力」的自願協議，
積壓是排隊論上的必然（到達率有機關、服務率靠心情）。參考實現曾積壓 19 條
後才機關化。不需要 cron 或常駐進程——**SessionStart 就是免費的觸發器流**，
用 anacron 語義（「今天還沒跑過就在第一個機會跑」）：
...
Stop
  └─ 當日仍未消費 → 記帳 consume_skipped（V1 階梯；兩週數據後定升級）
```

設計要點：
- **零打擾是存活條件**：任何每天搶佔用戶前十分鐘的機制終將被繞過；派工
  必須走背景子代理，與主任務並行。
- **殭屍回收**：早於今日的 consuming 是死掉的消費（計回 backlog 並點名）。
- **有人在場**：與 cron 相比，anacron 派工永遠發生在有人的 session 裡——
  順帶解決「無人值守 session 改 harness 自己」的風險。
- 純問答 session 可聲明不派（記帳照落，升級評估時校正分母）。
