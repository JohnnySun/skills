...
description: 跨模型層級的提示詞設計與改寫方法。Use when：撰寫或改寫派給子代理／其他模型的 prompt、把舊模型的 prompt/skill/system prompt 遷移到新模型、診斷「模型不聽話／太囉嗦／思考太淺／過度工程／虛報進度／review 召回率下降」類提示問題、為 Claude/GPT/開源模型等不同供應商調整同一任務的提示、在多模型可選時依性能證據與 token 成本選型，或評估某段提示對目標模型是否過度規範。跨工程通用，不綁定任何特定 repo 或供應商。
---
...
2. **在哪執行**——執行面決定「完成定義」要寫多死：
   ...
   - **產品 system prompt**：被調用百萬次、每個 token 都是成本 → 走模板 +
     變數，改動必須配 eval 量測。
3. **為何選它**——當平台允許手選多個模型，先確認任務能力門檻與獨立性能
   證據，再在合格候選中比較模型所有者的有效 token 成本；見
   [能力合格後的成本路由](references/model-routing.md)。

## 三層速查

| 層級 | 代表（能力類，非窮舉） | 模型缺什麼 | 提示要給 | 提示要刪 |
|---|---|---|---|---|
| **Frontier-agentic** | 經歸層探針或等價的 per-model 行為證據確認，能在拆腳手架下穩定完成長自主工作的模型 | 意圖、邊界、停止條件 | 為什麼要做（why）、驗收標準、什麼**不該**做、進度聲明需錨定工具證據、自主權聲明（何時該停下問人） | CoT 步驟清單、逐條行為列舉、強制進度節拍、推理復述要求、上下文餘量倒數 |
...
價格不是 tier：tier 決定提示策略與任務最低能力門檻；成本只在模型已被任務相關
的獨立證據或本地 eval 證明合格後才做決勝。平台指定的 Claude Code 或 Codex
官方預設不被覆蓋。不要從單一綜合 Index、模型所有者自家 benchmark，或 input
單價直接推導「性價比最高」；按 [能力合格後的成本路由](references/model-routing.md)
核對精確配置、harness、token 假設與模型所有者的價格來源。
...
| 多模型可選，且要以性能證據與 token 成本做路由 | [references/model-routing.md](references/model-routing.md) |
...
5. **能力先於成本**：先用可比的獨立性能證據或本地 eval 決定候選；只有
   候選都達到任務門檻時，才用模型所有者的有效 token 成本打破平手。
6. **解釋為什麼，不堆 MUST**：對主力層以上，一句說明意圖的話勝過三條
