...
若工程提供 owner overlay，先讀完整 runtime model ID 並按 overlay 對號入座；
能力層決定提示厚度，owner overlay 決定現役派工檔位。兩者不能互相覆寫。
...
| 已獲派工授權後，選現役角色/模型/effort 並套用驗收邊界 | [references/model-routing.md](references/model-routing.md) |
...
6. **派工路由不授權派工**：repo 治理決定能否委派；owner overlay 只在已獲
   授權後選模型與約束。GPT 施工結果必須由獨立 director 親跑可信集驗收。
